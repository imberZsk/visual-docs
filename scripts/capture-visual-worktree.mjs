...
function createRepo(name, files) {
  ...
  run(
    ...
    [
      ...
      'add',
      'origin',
      `https://gitlab.example.com/platform/${name}.git`,
    ],
    repoPath
  )
...
function createWorktree(repoPath, projectName, branchName, note) {
  ...
  run(
    'git',
    ['worktree', 'add', '-q', '-b', branchName, targetPath, 'main'],
    repoPath
  )
...
function prepareDemoData() {
  ...
  const frontendRepo = createRepo('frontend-shell', {
    ...
    'package.json':
      '{"scripts":{"dev":"vite --host 0.0.0.0 --port 5275","test":"vitest run"},"dependencies":{"antd":"^5.22.5"}}\n',
  ...
  const apiRepo = createRepo('api-gateway', {
    ...
    'package.json':
      '{"scripts":{"dev":"node server.js","test":"node --test"},"dependencies":{"fastify":"^5.0.0"}}\n',
  ...
  createWorktree(
    frontendRepo,
    'frontend-shell',
    'feat/settlement-panel',
    '前端结算页正在联调，保留一处未提交改动用于截图。'
  )
  createWorktree(
    apiRepo,
    'api-gateway',
    'feat/settlement-api',
    '接口聚合层已补充演示改动，展示任务下多仓库状态。'
  )
  createWorktree(
    miniappRepo,
    'miniapp-client',
    'feat/settlement-miniapp',
    '小程序客户端同步调整结算展示逻辑。'
  )
  ...
  writeJson(join(dataDir, 'task-links.json'), {
    [taskName]: [
      {
        name: 'Jira FEAT-231',
        url: 'https://jira.example.com/browse/FEAT-231',
      },
  ...
  writeJson(join(dataDir, 'task-history.json'), [
    {
      ...
      link: [
        {
          name: 'Jira BUG-118',
          url: 'https://jira.example.com/browse/BUG-118',
        },
      ],
      status: 'released',
...
function runCapture() {
  ...
  execFileSync(electronBin, [captureMainPath], {
    cwd: docsRoot,
    env,
    stdio: 'inherit',
  })
